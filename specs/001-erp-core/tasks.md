...
- [X] T001 Create Python project layout: `dodoo/`, `tests/unit/`, `tests/integration/`,
  `tests/e2e/`, `docs/adr/`, `dodoo/addons/base/` with `__init__.py` files in each package
- [X] T002 Write `pyproject.toml`: declare dependencies (fastapi, sqlalchemy[asyncio], asyncpg,
...
- [X] T003 [P] Write `.env.example` with `DATABASE_URL`, `DATABASE_MIGRATION_URL`,
  `SESSION_EXPIRY_HOURS`, `ADDONS_PATH`, `LOG_LEVEL` (no real values — example only)
- [X] T004 [P] Write `.github/workflows/ci.yml`: jobs for ruff lint, ruff format check,
...
- [X] T005 [P] Write `dodoo/core/exceptions.py`: define base exception hierarchy —
...
- [X] T006 Implement `dodoo/core/db.py`: async SQLAlchemy engine factory
...
- [X] T007 [P] Implement `dodoo/core/logging.py`: configure structlog (or stdlib logging with
  ...
  `LOG_LEVEL != DEBUG`
- [X] T008 [P] Write `tests/conftest.py`: pytest fixture `pg_engine` that spins up a
...
- [X] T009 Verify structured logging integration: write a single unit test in
...
- [X] T010 [P] Declare `pip-audit` in dev deps section of `pyproject.toml`; document approved
  licenses (MIT, Apache-2.0, BSD, PSF) in `pyproject.toml` `[tool.licenses]` table;
  note: CI integration is owned by T004
- [X] T011 [P] Wire `dodoo/core/db.py` startup into `dodoo/__init__.py` `Environment` stub:
...
- [X] T012 [P] Write `dodoo/__main__.py`: entry point for `python -m dodoo`; stub `server`
  and `module` subcommand parsers (implementations added in US2/US3 phases); parse
  `--host`, `--port`, `--database-url` for server subcommand
- [X] T013 [P] File ADRs for all 9 research decisions in `docs/adr/`: one `.md` file per
...
- [X] T014 [P] Implement `dodoo/core/bootstrap.py`: `bootstrap(ddl_conn, dml_conn)` function
...
- [X] T015 [P] Implement Python model classes for core registry tables in
...
- [X] T016 [P] [US1] Write unit tests for all field types in `tests/unit/test_fields.py`:
...
- [X] T017 [P] [US1] Write unit tests for model registry in `tests/unit/test_registry.py`:
...
- [X] T018 [P] [US1] Write unit tests for domain filter compiler in `tests/unit/test_query.py`:
...
- [X] T019 [US1] Implement `dodoo/core/fields.py`: `Field` base class with attrs
...
- [X] T020 [US1] Implement relational fields in `dodoo/core/fields.py`: `Many2one(relation)`,
...
- [X] T021 [US1] Implement `dodoo/core/registry.py`: `ModelRegistry` class with `register(cls)`
...
- [X] T022 [US1] Implement `BaseModel` metaclass in `dodoo/core/models.py`: collect `Field`
...
- [X] T023 [US1] Implement STI support in `dodoo/core/registry.py`: when a model declares
...
- [X] T024 [US1] Implement `dodoo/core/migration.py`: `MigrationRunner.install(model, conn)`:
  ...
  transaction
- [X] T025 [US1] Implement `dodoo/core/query.py`: `compile_domain(model, domain) ->
...
- [X] T026 [US1] Implement CRUD methods on `BaseModel` in `dodoo/core/models.py`:
...
- [X] T027 [US1] Wire `Environment.__getitem__` in `dodoo/__init__.py`: `env["model.name"]`
...
- [X] T028 [US1] Write integration tests for CRUD + STI in `tests/integration/test_orm_crud.py`:
  ...
  returns all rows
- [X] T029 [US1] Write integration tests for migration runner in
...
- [X] T030 [P] [US2] Write unit tests for manifest parsing and discovery in
...
- [X] T031 [US2] Implement `dodoo/modules/loader.py`: `AddonLoader` class;
...
- [X] T032 [US2] Implement `AddonLoader.resolve_order(names) -> list[str]` in
...
- [X] T033 [US2] Implement `dodoo/modules/installer.py`: `ModuleInstaller.install(name, env)`:
...
- [X] T034 [US2] Implement `ModuleInstaller.upgrade(name, env)` in
...
- [X] T035 [US2] Create `dodoo/addons/base/__manifest__.py`:
...
- [X] T036 [US2] Wire `python -m dodoo module install <name>` in `dodoo/__main__.py`:
...
- [X] T037 [US2] Write integration tests for module loader in
...
- [X] T038 [P] [US3] Write integration tests for JSON-RPC 2.0 in
...
- [X] T039 [US3] Implement `dodoo/http/middleware.py` — correlation ID + session validator
...
- [X] T040 [US3] Implement `dodoo/http/routing.py`: `RouteRegistry` singleton; `@http.route(
  ...
  from override
- [X] T041 [US3] Implement `dodoo/http/jsonrpc.py`: `POST /jsonrpc` FastAPI handler;
...
- [X] T042 [US3] Register `common` service methods in `dodoo/http/jsonrpc.py`:
...
- [X] T043 [US3] Register `object` service in `dodoo/http/jsonrpc.py`:
  ...
  **kwargs)`, returns serialised result; access rule enforcement applied by ORM layer
  (wired in US4)
- [X] T044 [US3] Implement `dodoo/http/app.py`: `create_app(env) -> FastAPI`; add CORS
...
- [X] T045 [P] [US3] Register base module REST endpoints in
...
- [X] T046 [US3] Wire `python -m dodoo server --host --port` in `dodoo/__main__.py`:
...
- [X] T047 [P] [US4] Write integration tests for session lifecycle in
...
- [X] T048 [P] [US4] Write integration tests for ir.rule enforcement in
...
- [X] T049 [US4] Implement `dodoo/addons/base/models/res_groups.py`: `res.groups` model with
  `Char` fields `name` (required), `full_name`, `category`; register with base module
- [X] T050 [US4] Implement `dodoo/addons/base/models/res_users.py`: `res.users` model with
  ...
  default field list
- [X] T051 [US4] Implement `dodoo/addons/base/models/ir_session.py`: `ir.session` model with
...
- [X] T052 [US4] Implement `dodoo/addons/base/models/ir_rule.py`: `ir.rule` model with
...
- [X] T053 [US4] Implement `dodoo/auth/session.py`: `SessionManager.authenticate(env, login,
...
- [X] T054 [US4] Implement `dodoo/auth/access.py`: `AccessEnforcer.get_applicable_rules(env,
  ...
  deny the operation
- [X] T055 [US4] Wire `AccessEnforcer` into `BaseModel` CRUD in `dodoo/core/models.py`:
  ...
  (no active session)
- [X] T056 [US4] Seed base data in `dodoo/addons/base/data/base_data.py`: create
...
- [X] T057 [US4] Wire real `SessionManager.validate()` into middleware `session_validator` in
...
- [X] T058 [P] Write e2e test in `tests/e2e/test_full_flow.py`: two scenarios — (1) library
...
- [X] T059 [P] Write benchmark tests in `tests/benchmarks/test_performance.py`: insert 100k
...
- [X] T060 Security audit: review each subsystem against OWASP Top 10 checklist (document
...
- [X] T061 [P] Observability validation: verify every HTTP request produces a structured log
...
- [X] T062 [P] Verify `quickstart.md` scenarios 1–7 all produce expected output on a clean
  install; update any paths or commands that diverged during implementation
- [X] T063 Run full CI gate sequence and confirm all pass with exit code 0: `ruff check .`,
