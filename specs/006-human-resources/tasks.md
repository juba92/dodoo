...
- [ ] T018 (BLOCKED on T041 `hr-menu.js` + US1 view modules) Register HR + Fleet routing in `dodoo/addons/web/static/app.js`: add `#/hr/*` and `#/fleet/*` rows to `_ROUTES`, `_paramsFromHash`, `_labelFromHash`; add `_renderHrMenu` / `_renderFleetMenu` (modelled on `_renderAccountingMenu`) dispatched from `_renderSidebar`; bump `CLIENT_BUILD`
