"""Localization test fixtures: installs base+account+web+localization once per session.

Unit + integration tests for the Localization & Settings feature live in this directory
(mirrors ``tests/accounting/``). End-to-end Playwright tests stay in ``tests/e2e/``.
"""
...
def modules_installed(db_url):
    """Install base+account+web+localization once per session in a dedicated loop."""
    import asyncio
    ...
    async def _install():
        ...
        await installer.install("localization")
        await env.close()
