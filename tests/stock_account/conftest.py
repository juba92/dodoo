"""Stock Account test fixtures: installs base+web+product+stock+account+stock_account once per
session (mirrors tests/hr/)."""
...
def modules_installed(db_url):
    """Install base+web+product+stock+account+stock_account once per session."""
    import asyncio
    ...
    async def _install():
        ...
        await installer.install("stock_account")
        await env.close()
