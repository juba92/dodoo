"""009-customer-database: accounting-owned partner fields and the AR ledger.

``res.partner`` lives in ``base``, which declares only the general contact fields
(``name``/``email``/``phone``/``vat``/billing address). ``customer_rank`` and
``property_currency_id`` are ``account``-owned raw columns (ADR-046, the same
cross-addon idiom already used for ``property_account_receivable_id``/
``property_payment_term_id``) — not declared ``Field``s on ``base``'s
``ResPartner`` class, so the generic ``ResPartner.create``/``write`` silently
drops them. This module is where ``account`` reads and writes its own columns.
"""
...
_PROPERTY_COLUMNS = ("customer_rank", "property_currency_id", "property_payment_term_id")
