/**
 * Vendor form (010-vendor-database, US1/US3) — create/edit a vendor's master
 * data (name, contact info, billing address, VAT, default payment
 * terms/currency) plus a read-only Accounts Payable panel (balance +
 * bill/credit-note/payment history with drill-down), modelled file-for-file
 * on `customer-form.js`. Uses the dedicated `/account/vendor` create/update
 * routes (not `/account/partner`, which is fixed to `CustomerCreate`/
 * `CustomerUpdate` — ADR-051) and the shared `GET /account/partner/{id}` read
 * route.
 */
...
const TYPE_LABEL = { bill: 'Bill', credit_note: 'Credit Note', payment: 'Payment' };
...
export async function render(container, params) {
  ...
  let rec = {
    ...
    property_supplier_payment_term_id: null, property_currency_id: null,
  };
  ...
  title.textContent = isNew ? t('New Vendor') : (rec.name || '');
  container.appendChild(title);

  const banner = document.createElement('div');
  banner.id = 'vendor-status';
  ...
  const nameInput = _text(grid, 'vend-name', t('Name'), rec.name || '', true);
  const emailInput = _text(grid, 'vend-email', t('Email'), rec.email || '', false);
  const phoneInput = _text(grid, 'vend-phone', t('Phone'), rec.phone || '', false);
  const streetInput = _text(grid, 'vend-street', t('Street'), rec.street || '', false);
  const cityInput = _text(grid, 'vend-city', t('City'), rec.city || '', false);
  const zipInput = _text(grid, 'vend-zip', t('ZIP'), rec.zip || '', false);
  ...
  _wrapField(grid, 'vend-country', t('Country'), countrySel, false);
  ...
  _wrapField(grid, 'vend-state', t('State/Region'), stateSel, false);

  const vatInput = _text(grid, 'vend-vat', t('Tax/VAT Number'), rec.vat || '', false);
  ...
  termSel.value = rec.property_supplier_payment_term_id ? String(rec.property_supplier_payment_term_id) : '';
  _wrapField(grid, 'vend-term', t('Payment Terms'), termSel, false);
  ...
  _wrapField(grid, 'vend-currency', t('Currency'), currSel, false);
  ...
  discardBtn.onclick = () => App.navigate('#/accounting/vendors');
  ...
  if (!isNew) {
    ...
    deleteBtn.onclick = async () => {
      try {
        ...
        App.navigate('#/accounting/vendors');
      } catch (err) {
  ...
  saveBtn.onclick = async () => {
    const vals = {
      ...
      property_supplier_payment_term_id: termSel.value ? parseInt(termSel.value, 10) : null,
    ...
    if (vals.vat) {
      try {
        ...
        if (others.length) _flash(banner, 'warning', t('Another vendor already uses this Tax/VAT number.'));
    ...
    try {
      if (isNew) {
        const newId = await api.post('/account/vendor', vals);
        App.navigate('#/accounting/vendor/' + newId);
      } else {
        if (archCb) await api.rpc('res.partner', 'write', [[parseInt(id, 10)], { active: !archCb.checked }]);
        await api.patch(`/account/vendor/${id}`, vals);
  ...
  // ── Accounts Payable panel (existing vendors only) ─────────────────────────
  if (!isNew) {
    const heading = document.createElement('h3');
    heading.style.margin = '24px 24px 0';
    heading.textContent = t('Accounts Payable');
    container.appendChild(heading);

    try {
      const ledger = await api.get(`/account/partner/${id}/ap-ledger`);
      ...
      if (!ledger.lines || !ledger.lines.length) {
        ...
        td.colSpan = 5;
        td.textContent = t('No bills, credit notes, or payments yet.');
        tr.appendChild(td);
      ...
      el.textContent = t('Failed to load Accounts Payable data') + ': ' + err.message;
      container.appendChild(el);
    }
  }
}
